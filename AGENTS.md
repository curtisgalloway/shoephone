...
| Notifications and ledger | Content-free push to an ntfy-style topic the operator runs (`[notify]` in the config: url, optional bearer token, optional click URL), sent for a new request, an opened window, and a killed window (fixed text each; declines, timeouts and issues stay silent), never retried; the page fetches details from the daemon. Every enrolled device hears every opened window, so an approval from one device is visible on the others. Ledger is an append-only file on the daemon plus the page's history view until an app exists. |
